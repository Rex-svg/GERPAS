...
export async function GET(req: Request) {
  try {
    ...
    return new Response(JSON.stringify({ warehouse: data }), { status: 200 });
  } catch (err: unknown) {
...
export async function PUT(req: Request) {
  try {
    ...
    const { data, error } = await supabase
      .from("warehouses")
      .update({ name: body.name, address: body.address || null })
      .eq("id", id)
      .eq("factory_id", auth.factory.id)
      .select();
    if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500 });
    return new Response(JSON.stringify({ warehouse: data?.[0] || null }), { status: 200 });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    return new Response(JSON.stringify({ error: message }), { status: 500 });
  }
